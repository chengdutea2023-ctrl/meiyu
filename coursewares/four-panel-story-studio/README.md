...
这是接入“智美教育新生态业务底座”的网页 + 服务课件。学生录制一段故事灵感语音，课件将它整理成起承转合四格漫画脚本，生成 4 套新中式风格整页四格漫画候选，并把学生原声、字幕、背景音乐和选中的漫画合成短视频。

4 套候选默认对应 4 种美术方向：新中式水墨风格、新中式国潮风格、新中式工笔淡彩风格、新中式 Q 版可爱风格。
...
上传作品包括学生录音、4 套候选漫画图、选中漫画、WebVTT 字幕和最终 mp4 视频。视频合成接口支持 `bgmMood: "soft" | "fast"`，分别生成柔和配乐和轻快配乐；背景音乐由 ffmpeg 本地生成，不调用第三方音乐接口。
