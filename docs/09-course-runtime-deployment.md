...
管理员后台“课程课件”支持选择课程目录上传。第一版上传规则：

- 管理员先创建课程，设置 `slug`、课程名、运行方式和入口。
- 点击“上传课件”，选择课程目录。
- 系统把文件写入服务器课程目录。
- 如果目录里没有 `manifest.json`，系统会自动生成基础 manifest。
- 可选择“上传后发布课程”。
- Node 课件只上传文件，不自动以 root 权限运行；管理员审核后再配置独立服务。
...
COURSE_UPLOAD_MAX_BYTES=52428800
